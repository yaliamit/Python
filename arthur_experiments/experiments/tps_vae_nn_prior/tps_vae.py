...
def build_model(input_var=None, input_shape=(None, ), n_latent=2,
                n_hidden=128, n_layers=1, batch_size=100, n_spatial=5, n_hidden_prior=10):
    print("Building model ...")
    network, (z_mu_net, z_ls_net), z_net, u_net, (u_mu_net, u_logsigma_net) = build_transf_var_autoencoder(
        input_var, input_shape, n_latent=n_latent, n_hidden=n_hidden,
        n_layers=n_layers, n_latent_spatial=n_spatial, n_hidden_prior=n_hidden_prior)

    def loss_fn(y_pred, y_true, z_mu, z_ls, u_mu, u_ls, val=False):
            eps = 1e-6
            y_pred = y_pred.clip(eps, 1 - eps)
            kl_div_u = 1 + 2 * u_ls - T.sqr(u_mu) - T.exp(2 * u_ls)
    ...
    outputs = lasagne.layers.get_output([network, z_mu_net, z_ls_net, u_mu_net, u_logsigma_net])
    ...
    loss = loss_fn(prediction, output_var, z_mu, z_ls, u_mu, u_ls)
    ...
    outputs = lasagne.layers.get_output(
        [network, z_mu_net, z_ls_net, z_net, u_net, u_mu_net, u_logsigma_net], deterministic=True)
    ...
    test_loss = loss_fn(test_prediction, output_var, z_mu, z_ls, u_mu, u_ls, True)
    ...
    param_fn = theano.function([input_var], u)
...
def build_transf_var_autoencoder(input_var=None, input_shape=(None, ),
                                 ...
                                 n_latent=2, n_hidden_prior=10, n_latent_spatial=5):
    ...
    u_mean = lasagne.layers.DenseLayer(
        x, num_units=n_latent_spatial, nonlinearity=lasagne.nonlinearities.tanh)

    u_logsigma = lasagne.layers.DenseLayer(
        x, num_units=n_latent_spatial,
        nonlinearity=lambda a: T.nnet.relu(a + relu_shift) - relu_shift)

    u = GaussianSampleLayer(u_mean, u_logsigma)
    u_hid = u
    u_hid = lasagne.layers.DenseLayer(u_hid, num_units=n_hidden_prior, nonlinearity=lasagne.nonlinearities.rectify)

    transf_param = lasagne.layers.DenseLayer(
        u_hid, num_units=2 * n_control, nonlinearity=lasagne.nonlinearities.tanh)
    ...
    return l_out, (z_mean, z_logsigma), z, u, (u_mean, u_logsigma)
