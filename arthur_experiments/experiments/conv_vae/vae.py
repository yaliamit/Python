...
def build_model(input_var=None, input_shape=(None, ), n_latent=2,
    ...
    print("Building model Conv VAE ...")
    network, (z_mu_net, z_ls_net), z_net = build_transf_var_autoencoder(
    ...
    decode_fn = theano.function([z_vect], generated_x)
...
def build_transf_var_autoencoder(input_var=None, input_shape=(None, ),
    ...
    x = lasagne.layers.Conv2DLayer(x, n_filter, 3, pad='same', nonlinearity=activation_conv) # out = 28x28
    ...
    x = lasagne.layers.Conv2DLayer(x, n_filter, 3, pad='same', nonlinearity=activation_conv)
    x = lasagne.layers.Conv2DLayer(x, 1, 3, pad='same', nonlinearity=activation_conv) # out 14x14
