...
def build_model(input_var=None, input_shape=(None, ), n_latent=2,
    ...
    input_sig = T.vector()
    network, (z_mu_net, z_ls_net), z_net, trans_param_net, (u_mean_net, u_logsigma_net), u_mu_pr_net, u_ls_pr_net = build_transf_var_autoencoder(
        input_var, input_shape, n_latent=n_latent, n_hidden=n_hidden,
        n_layers=n_layers, input_sig = input_sig)

    def loss_fn(y_pred, y_true, z_mu, z_ls, u_mu, u_ls, u_mu_pr, u_ls_pr, val=False):
            eps = 1e-6
            y_pred = y_pred.clip(eps, 1 - eps)
            u_ls_pr_scal = T.stack(batch_size * [u_ls_pr], axis=0)
            u_mu_pr_scal = T.stack(batch_size * [u_mu_pr], axis=0)
            kl_div_u = 1 + 2 * (u_ls - u_ls_pr_scal)
            kl_div_u -= T.sqr(u_mu - u_mu_pr_scal) / T.exp(2 *u_ls_pr_scal)  + T.exp(2 * (u_ls - u_ls_pr_scal))
    ...
    outputs = lasagne.layers.get_output([network, z_mu_net, z_ls_net, u_mean_net, u_logsigma_net, u_mu_pr_net, u_ls_pr_net])
    ...
    loss = loss_fn(prediction, output_var, z_mu, z_ls, u_mu, u_ls, u_mu_pr, u_ls_pr)

    params = lasagne.layers.get_all_params(network, trainable=True)
    params_sigma = lasagne.layers.get_all_params(u_ls_pr_net, trainable=True)
    params_mu = lasagne.layers.get_all_params(u_mu_pr_net, trainable=True)
    params = params + params_sigma + params_mu
    ...
    test_loss = loss_fn(test_prediction, output_var, z_mu, z_ls, u_mu, u_ls, u_mu_pr, u_ls_pr, True)

    print("Compiling functions ...")
    sigma_default = np.array(5 * [0], dtype="float32")
    ...
    mu_spatial = u_mu_pr_net.b + np.array([0, 1, 1, 0, 0], dtype='float32')
    return decode_fn, encode_fn, train_fn, val_fn, predict_fn, param_fn, mu_spatial, u_ls_pr_net 
...
class AffineParams(lasagne.layers.Layer):
    def get_output_shape_for(self, input_shapes):
        return input_shapes[:-1] + (6, )

    def get_output_for(self, inputs, **kwargs):
        cos_theta = T.cos(inputs[:, 0])
        sin_theta = T.sin(inputs[:, 0])
        # sx = T.exp(inputs[:, 1])
        # sy = T.exp(inputs[:, 2])
        sx = inputs[:, 1]
        sy = inputs[:, 2]
...
def build_transf_var_autoencoder(input_var=None, input_shape=(None, ),
    ...
    prior_input = lasagne.layers.InputLayer(shape=(5,),
        input_var=input_sig)
    u_logsigma_pr = lasagne.layers.BiasLayer(
        prior_input, b=lasagne.init.Constant(-0.5), shared_axes=1)
    u_mu_pr = lasagne.layers.BiasLayer(
        prior_input, b=lasagne.init.Constant(0), shared_axes=1)

    u_mean = lasagne.layers.DenseLayer(
        x, num_units=5, nonlinearity=None)#lasagne.nonlinearities.tanh
    ...
    u = lasagne.layers.standardize(
        u, np.array([0, -1, -1, 0, 0], dtype='float32'),
        np.ones(5, dtype='float32'))
    trans_par = AffineParams(u)
    l_out = lasagne.layers.TransformerLayer(x, trans_par)

    return l_out, (z_mean, z_logsigma), z, u, (u_mean, u_logsigma), u_mu_pr, u_logsigma_pr
