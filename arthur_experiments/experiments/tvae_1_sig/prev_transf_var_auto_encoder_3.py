...
class AffineParams(lasagne.layers.Layer):
    ...
    def get_output_for(self, inputs, **kwargs):
        ...
        out = T.stack([inputs[:, 1] * cos_theta,
                       -inputs[:, 1] * sin_theta,
                       inputs[:, 3],
                       inputs[:, 2] * sin_theta,
                       inputs[:, 2] * cos_theta,
...
def build_transf_var_autoencoder(input_var=None, input_shape=(None, ),
    ...
    u_mean = lasagne.layers.DenseLayer(
        x, num_units=5, nonlinearity=lasagne.nonlinearities.tanh)
    ...
    u = lasagne.layers.standardize( # order theta, Sx, Sy, Tx, Ty
        u, np.array([0, -1, -1, 0, 0], dtype='float32'), #[0.3, -1, -1, 0, 0]
