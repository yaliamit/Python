...
def iterate_on_batches(func,X,y,batch_size,typ='Test',fac=False, agg=False, network=None, pars=None, iter=None):
    ...
    for batches,batch in enumerate(iterate_minibatches_new(X, yy, batch_size, shuffle=shuffle)):
        ...
        if (typ=='Train'):
           print(tout[4].shape,np.std(tout[4]))
           print(tout[5].shape,np.std(tout[5]))
           print(tout[6].shape, np.std(tout[6]))
           print(tout[7].shape, np.std(tout[7]))
           print(tout[8].shape, np.std(tout[8]))
           print(tout[9].shape, np.std(tout[9]))
    ...
    if (pars is not None and 'Classes' in pars and pars['Classes'] is not None and typ!='Train'):
        lcl=list(np.unique(pars['Done_Classes']+pars['Classes']))
        yind=np.in1d(yy,lcl)
...
def main_new(NETPARS):
    ...
    #np.random.seed(np.long(NETPARS['seed']))
