...
for j in np.arange(0,1,1):
    ...
    ss='python runber.py net=_pars'+pfile+' hinge=1. num_train='+nt+' use_existing=True eta_init=.001 ' \
       'eta_current=.001 batch_size=5000 start=1 mult=1 mod_net='+mod_net+' update='+update+' num_epochs=500 ' \
    ...
    os.system(ss)
    ss='python runber.py net=_pars'+pfile+' hinge=1. num_train='+nt+' use_existing=True eta_init=.001 ' \
       'eta_current=.001 batch_size=5000 NOT_TRAINABLE=[newdens1,] start=1 mult=1 mod_net='+mod_net+' update='+update+' num_epochs=500 ' \
