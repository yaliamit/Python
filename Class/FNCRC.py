...
for j in np.arange(0,10,1):
    ...
    ss='python runber.py net=_pars'+pfile+' hinge=1. num_train='+nt+' use_existing=True eta_init=.1 ' \
        'eta_current=.1 batch_size=5000 start=1 mult=1 update='+update+' num_epochs='+ne+ \
    ...
    # os.system(ss)
    ss='python runber.py net=_pars'+pfile+' hinge=1. num_train='+nt+' use_existing=True eta_init=.1 ' \
       'eta_current=.1 batch_size=5000 NOT_TRAINABLE=[newdens1,] start=1 mult=1 update+'+update+' num_epochs='+ne+ \
