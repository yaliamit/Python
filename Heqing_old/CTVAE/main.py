...
model = CTVAE(h, w, h_dim, z_dim, u_dim, mb_size, device, args.transformation).to(device)
...
def test(epoch):
    ...
    with torch.no_grad():
        for _, (data, target) in enumerate(te):
            ...
            recon_batch, scores, zmu, zlogvar, umu, ulogvar = model(data)
            recon_loss, kl = loss_V(recon_batch, data, zmu, zlogvar, umu, ulogvar)
            cerror = criterion(scores,target)
            loss = recon_loss + kl + args.lamda * cerror
            total_correct += classify(scores, target)
            test_recon_loss += recon_loss.item()
            
    test_recon_loss /= (len(te) * mb_size)
    acc = 100. * total_correct / (len(te) * mb_size)
...
def train(epoch):
    ...
    print('====> Epoch: {} Classification loss: {:.4f}'.format(
          epoch, c_loss / (len(tr)*mb_size)))
    print('====> Epoch: {} Classification accuracy: {:.4f}'.format(
          epoch, 100. * total_correct / (len(tr)*mb_size)))
