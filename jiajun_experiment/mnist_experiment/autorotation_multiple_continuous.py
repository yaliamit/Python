...
def build_cnn(input_var=None):
    ...
    network_middle_output = lasagne.layers.ReshapeLayer(network, shape = (([0], 256)))
...
# all_weights = np.load("../data/mnist_CNN_params_drop_out_semi_Chi_Nov28.npy")
...
rotated_test_batches = X_test_rotated.shape[0] // 100
num_epochs = 2000
for epoch in range(num_epochs):
    ...
    for batch in iterate_minibatches(X_train, y_train, 100, shuffle = True):
        ...
        for p in range(4):
            angles = list(np.random.randint(low = -20 + 10 * p, high = -10 + 10 * p, size = 100))

            function_arguments += (np.array([rotateImage(inputs[i], angles[i]) for i in range(100)], dtype = np.float32),)
            # function_arguments += (np.array([rotateImage(inputs[i], angles[i]) for i in range(100)], dtype = np.float32),)

        current_cost, current_accuracy, _, _ = train_fn(*function_arguments)
    ...
    if epoch % 50 == 0:
        ...
        for batch in iterate_minibatches(X_test, y_test, 100, shuffle = False):
            ...
            current_cost, current_accuracy, _, _ = val_fn(*function_arguments)
            

            function_arguments = (inputs, targets)
            for i in range(4):
                function_arguments += (inputs, )
            _, current_accuracy_on_original, _, _ = val_fn(*function_arguments)
        ...
        print("test accuracy on original: \t\t{:.6f}".format(test_accuracy_on_original / test_batches))


        rotated_test_accuracy = 0
        rotated_original_test_accuracy = 0
        for batch in iterate_minibatches(X_test_rotated, y_test_rotated, 100, shuffle = False):
            inputs, targets = batch
            # degree_list = [-20, -10, 10, 20]
            import random
            # angles = [random.choice(degree_list) for i in range(100)]
            ...
            _, current_accuracy, _, _ = val_fn(*function_arguments)
            

            rotated_test_accuracy += current_accuracy
            rotated_original_test_accuracy += current_original_accuracy
             
        print("Test Results:  ")
        print("Rotated Test accuracy:\t\t{:.6f}".format(rotated_test_accuracy / rotated_test_batches))
        print("Rotated Original Test Accuracy:\t\t{:.6f}".format(rotated_original_test_accuracy / rotated_test_batches))
