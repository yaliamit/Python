...
def iterate_minibatches(inputs, targets, batchsize, classNum = 10, shuffle=False):
    ...
    for start_idx in range(0, len(inputs) - batchsize + 1, batchsize):
        ...
        yield inputs[excerpt], np.array(binary_targets,dtype = np.float32), targets[excerpt]
...
def main():
    ...
    target_llh_var = T.fmatrix('targets_llh')
    target_prediction_var = T.ivector('targets_classification')

    # Create neural network model (depending on first command line parameter)

    network, fc = build_cnn(input_var)
    ...
    llh_output, classification_output = lasagne.layers.get_output(network)
    loss_llh = lasagne.objectives.multi_negative_llh(llh_output, target_llh_var)
    loss_llh_mean = loss_llh.mean()
    ...
    test_llh_output, test_classification_output = lasagne.layers.get_output(network, deterministic=True)
    test_loss_llh = lasagne.objectives.multi_negative_llh(test_llh_output, target_llh_var)
    ...
    for epoch in range(num_epochs):
        ...
        for batch in iterate_minibatches(X_train, y_train, 50, 10, shuffle=True):
            inputs, targets_llh, targets_prediction = batch
            #current_loss_mean, current_loss, fc_output = train_fn(inputs, targets)
            current_result = train_fn(inputs, targets_llh, targets_prediction)
    ...
    #             print(current_result)
        ...
        if epoch % 2 == 0: 
            ...
            for batch in iterate_minibatches(X_test, y_test, 50, 10, shuffle=False):
                inputs, targets_llh, targets_prediction = batch
                valuationResult = val_fn(inputs, targets_llh, targets_prediction)
