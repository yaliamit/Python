...
rotated_network, rotated_network_middle, weight_decay = build_rotation_cnn(rotated_input_var)
...
for epoch in range(num_epochs):
    ...
    for batch in iterate_minibatches(X_train, y_train, 100, shuffle = True):
        ...
        current_cost, current_weight_decay, current_accuracy = train_fn(inputs, rotated_inputs, targets)
        accuracy += current_accuracy
        total_cost += current_cost
        total_weight_decay += current_weight_decay
        
    # Then we print the results for this epoch:
    print("Epoch {} of {} took {:.3f}s".format(
         epoch + 1, num_epochs, time.time() - start_time))
    print("training cost:\t\t{:.6f}".format(total_cost / train_batches))
    print("training weight_decay:\t\t{:.6f}".format(total_weight_decay / train_batches))
...
np.save("../data/mnist_rotated_network_Chi40_nin.npy", rotatedNetworkParams)
