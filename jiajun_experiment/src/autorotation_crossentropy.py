...
L = -T.mean(original_network_middle_output * T.log(rotated_network_middle_output) + (1 - original_network_middle_output) * T.log(1 - rotated_network_middle_output), axis = 1)
cost = T.mean(L)

updates = lasagne.updates.nesterov_momentum(cost, rotated_network_training_param, learning_rate = 0.1, momentum = 0.95)
...
for epoch in range(num_epochs):
    ...
    for batch in iterate_minibatches(X_train, y_train, 100, shuffle = True):
        ...
        angles = np.random.randint(low = -30, high = 30, size = 100)
...
np.save("../data/mnist_rotated_network_NY40.npy", rotatedNetworkParams)
