...
def load_mnist(section="training", offset=0, count=None, ret='xy', marg=0,
    ...
    if 'x' in ret:
        ...
        images = dd.util.pad(images, (0, marg, marg), value=0)
        #images = images[...,np.newaxis]
        # large_images=np.zeros((images.shape[0],images.shape[1]+2*marg,images.shape[2]+2*marg))
        # large_images[:,marg:marg+d0,marg:marg+d1]=images
        # images=large_images
        if x_dtype == np.uint8:
