...
class NewStructuredDot(gof.Op):
    # See doc in instance of this Op or function after this class definition.
    __props__ = ()

    def make_node(self, a, b, R):

        a = as_sparse_variable(a)
        R = as_sparse_variable(R)
    ...
    def grad(self, inputs, gout):
        ...
        (a, b, R) = inputs
        (g_out,) = gout
        da=structured_dot_grad(a, b, g_out)
        return [da, sparse.structured_dot(R.T, g_out), da]

    def infer_shape(self, node, shapes):
        return [(shapes[0][0], shapes[1][1])]

_new_structured_dot = NewStructuredDot()


def new_structured_dot(x, y, R):
    """
    ...
    if x_is_sparse_variable:
        return _new_structured_dot(x, y, R)
    else:
        assert y_is_sparse_variable
        return _new_structured_dot(y.T, x.T, R.T).T
