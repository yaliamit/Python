...
class rect_sym_scaled(object):

    def __init__(self, scale_in=.1):
        self.scale_in=scale_in

    def __call__(self,x):
        y=theano.tensor.nnet.relu(x*self.scale_in+1)-1
        z=-theano.tensor.nnet.relu(0,-y+1)+1
        return z
...
def get_nonlinearity(l,tinout,scale=1.):
    ...
    if ('rectify' in s1):
        ...
        f=make_net.rect_sym #_scaled(scale_in=scale_in)
    # elif ('rect_sym' in s1):
    #     f=rect_sym
    elif ('softmax' in s1):
...
def build_cnn_on_pars(input_var, PARS, input_layer=None, num_class=None):
    ...
    # input_la always has the same length as the initial input list
    for l in PARS['layers']:
        ...
        if ('parent' in l):
            ip=l['parent']
            if (type(ip)==list):
                # get coordinatewise maximum of parent layers
                if (not 'concatsum' in l['name']):
        ...
        if 'input' in l['name']:
            ...
            for lay in input_la:
                ...
                for lay in input_la:
                    ...
                    np.random.rand(1234)
                    W=np.float32(np.random.rand(input_dim,num_units))
                    np.save('W'+l['name'],W)
                    print(W[0,0],W[2,2])
