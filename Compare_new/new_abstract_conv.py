...
def conv2d(input,
    ...
    return conv_op(input, filters, R, Wzer, Rzer)
...
class AbstractConv(BaseAbstractConv):
    ...
    def make_node(self, img, kern, R, Wzer, Rzer):
        ...
        return Apply(self, [img, kern, R, Wzer, Rzer], [output])


    def perform(self, node, inp, out_):
        img, kern, R, Wzer, Rzer = inp
...
class AbstractConv2d(AbstractConv):
    ...
    def grad(self, inp, grads):
        bottom, weights, R, Wzer, Rzer = inp
        top, = grads
        WW=weights
        if (R.type.ndim==4):
            WW=R
        ...
        d_bottom = AbstractConv2d_gradInputs(self.imshp, self.kshp,
            ...
            WW, top, bottom.shape[-2:], add_assert_shape=False)
        ...
        return d_bottom, d_weights, d_R, d_Wzer, d_Rzer
