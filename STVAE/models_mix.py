...
class STVAE_mix(models.STVAE):
    ...
    def orthogo(self):
        ...
        aa=self.conv.weight.reshape(self.feats, self.filts * self.filts * self.input_channels)
    ...
    def mixed_loss_pre(self,x,data):
        ...
        if (not self.output_cont):
            ...
            for xx in x:
                dat = data.reshape(data.shape[0],-1)
                a = (dat - xx) * (dat - xx)
                a = torch.sum(a, dim=1)
                b = b + [a]
    ...
    def run_epoch(self, train, epoch,num_mu_iter, MU, LOGVAR,PI, d_type='test',fout=None):
        ...
        if (epoch>=self.lim and self.feats_back):
            print('Stopping feats back')
