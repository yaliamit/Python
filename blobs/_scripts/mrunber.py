...
if 'loc' in outname:
    ...
    f.write(ss+'\n')
    f.close()
    commands.check_output("scp runthingsBR.txt amit@bernie.uchicago.edu:/ga/amit/Python/blobs/.",shell=True)
    ...
    os.system("ssh amit@bernie.uchicago.edu \'cd /ga/amit/Python/blobs; ./runthingsBR.txt \' & ")

    ny='no'
    while (ny != ''):
        time.sleep(10)
        try:
            ss=commands.check_output('scp amit@aitken:Python/blobs/'+OUTNAME+' '+ lOUTNAME,shell=True)
            print(ss)
            ny=''
        except:
            ny='no'
